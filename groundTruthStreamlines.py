...
def nicelines(npath,lines_in):
    ...
    # testmask[:] = 0
    # testmask[halfdrt_nii.get_fdata() == 2] = 1
    # lines_inner = nonbuggytarget(lines,testmask, mask_nii.affine)
    #
    # testmask[:] = 0
    # testmask[halfdrt_nii.get_fdata() == 1] = 1
    # lines_outer= nonbuggytarget(lines,testmask,mask_nii.affine)
    #
    #
    # testmask[:]=0
    # testmask[((halfdrt_nii.get_fdata()==2) & ((angles_nii.get_fdata()==6) | (angles_nii.get_fdata()==1)))]=1
    # lines_inner_far = nonbuggytarget(lines,testmask, mask_nii.affine)
    #
    # testmask[:] = 0
    # testmask[((halfdrt_nii.get_fdata()==1) & ((angles_nii.get_fdata()==6) | (angles_nii.get_fdata()==1)))] = 1
    # lines_outer_far = nonbuggytarget(lines, testmask, mask_nii.affine)



    return lines_tang#, lines_inner, lines_outer, lines_inner_far, lines_outer_far
...
allmeans=np.zeros([8,5,8,4,2,2])

for i_io in range(3,4):#len(res)):
    print("i",i_io)
    for j_io in range(1,2):#len(drt)):
        print("i j", i_io, j_io)
        for k_io in range(0, 1):#len(ang_thr)):
            for l_io in range(-1,0):#len(w)):
                ...
                allmeans[i_io, j_io, k_io, l_io, 0, 0] = ntang_sens.mean()
                allmeans[i_io, j_io, k_io, l_io, 0, 1] = utang_sens.mean()

                allmeans[i_io, j_io, k_io, l_io, 1, 0] = ntang_sens.std()/np.sqrt(len(ntang_sens))
                allmeans[i_io, j_io, k_io, l_io, 1, 1] = utang_sens.std()/np.sqrt(len(utang_sens))
...
# for i in range(0,7):#len(res)):
#     for j in range(1,5):#len(drt)):
...
#         y=allmeans[i,j,:,-1,0,0]
#         x=ang_thr
#         ax[i,j].plot(x,y,color='blue')
#         y=allmeans[i,j,:,-1,0,1]
