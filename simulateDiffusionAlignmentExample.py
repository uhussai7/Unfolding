...
class tractographyExperiments:
    ...
    def simulate(self,w,drt,scale,res,beta):
        ...
        sim = simulateDiffusion.simulateDiffusion(phi, dphi, phiInv, Uparams, L1L2L3, windowFunction, bvals=bvals, bvecs=bvecs, N0=Nx)
...
for l in range(0,len(w)):
    for b in range(0,len(beta)):
        ...
        base = "/home/u2hussai/scratch/unfoldingSimulations/diffusionSimulations-Alignment-LambdaStep_scale_50_res-"
        npath=base + str(int(res[i] * 1000)) + "um_drt-" +str(int(drt[j] * 100)) +"_w-"+\
            str(int(round(w[l],2)*100))+"_angthr-" + str(int(ang_thr[k])) + "_beta-"+str(int(beta[b]*100))+"/"
        upath=npath+"Unfolded/"
        tracking_experiment=tractographyExperiments(npath,upath)
        tracking_experiment.simulate(w[l],drt[j],scale,res[i],beta[b])
